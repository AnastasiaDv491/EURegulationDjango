...
class BaseHistoryRequestQueryset(QuerySet):
    ...
    def _get_requests(self, start_date, end_date, assets, **kwargs):
        ...
        requests = (
            self.filter(**kwargs, **{f"{self.asset_field}__in": assets})
            .filter(
    ...
    def get_all_missing_intervals_per_asset(
        self, start_date, end_date, assets, **kwargs
    ):
        """
        ...
        for request in requests:
            ...
            asset_dict[self._get_instr_key(instr)].append(
        ...
        for key, ranges in asset_dict.items():
            ...
            for range_a, range_b in zip(
                chain([(None, start_date - timedelta(days=1))], ranges),
                ...
                if range_a[1] < range_b[0] - timedelta(days=1):
                    missing.append(
                        (range_a[1] + timedelta(days=1), range_b[0] - timedelta(days=1))
                    )
            if missing:
    ...
    def get_missing_interval_per_asset(self, start_date, end_date, assets, **kwargs):
        ...
        """
        missing = self.get_all_missing_intervals_per_asset(
            start_date, end_date, assets, **kwargs
        )
        return {key: (values[0][0], values[-1][1]) for key, values in missing.items()}

    def get_one_missing_interval_for_all_assets(
        self, start_date, end_date, assets, **kwargs
    ):
        """
        ...
        """
        intervals = self.get_missing_interval_per_asset(
            start_date, end_date, assets, **kwargs
        )
        if not intervals:
            return None
        return (
            min(r[0] for r in intervals.values()),
            max(r[1] for r in intervals.values()),
        )
    ...
    def update_request_table(self, start_date, end_date, assets, **kwargs):
        ...
        request_dict = dict()
        for request in requests:
            request_dict.setdefault(
                self._get_instr_key(getattr(request, self.asset_field)), []
            ).append((request.start_date, request.end_date))
