...
def merge_datframes(regulation, relation):
    ...
    df1 = relation[["source", "target", "relation"]].merge(regulation[["source"]], on =  "source", how = "inner")

    regulation = regulation.rename(columns = {"source":"target"})
    final_df = df1[["source", "target", "relation"]].merge(regulation[["target"]], on = "target", how = "inner")
